...
async def run_benchmark():
    ...
    # Load 7B Config Path
    config_7b_path = ROOT / "config_7b.yaml"

    # 1. Run Raw
    # Initialize with 7B config to ensure correct model load
    bot_raw = MalayaChatbot(config_path=str(config_7b_path))
    # Double check overrides (redundant but safe)
    if "model" not in bot_raw.config: bot_raw.config["model"] = {}
    bot_raw.config["model"]["provider"] = "ollama"
    bot_raw.config["model"]["name"] = "qwen2.5:7b"
    
    # 2. Run Malaya
    bot_malaya = MalayaChatbot(config_path=str(config_7b_path))
    if "model" not in bot_malaya.config: bot_malaya.config["model"] = {}
    bot_malaya.config["model"]["provider"] = "ollama"
    bot_malaya.config["model"]["name"] = "qwen2.5:7b"
    ...
    report_path = ROOT / "reports/benchmark_7b_baseline_logs.jsonl"
    ...
    print(f"\n⏱️  Total Time Taken: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
    
    # Save timing to summary
    summary["custom_metrics"] = {
        "total_time_seconds": total_time,
        "avg_time_per_case_seconds": total_time / len(cases)
    }
    with open(final_path, "w") as f:
        json.dump(summary, f, indent=2)
