#!/usr/bin/env python3
"""
Re-run only failed cases for Malaya V6 benchmark and update the existing log.

Usage:
  MALAYA_BENCHMARK_OUTPUT=reports/v3_benchmark/malaya_ai_v6.json \
  python scripts/benchmark_v6_rerun_failed.py
"""
...
DEFAULT_LOG = ROOT / "reports" / "v3_benchmark" / "malaya_ai_v6.json"
DEFAULT_FAILURES = ROOT / "reports" / "malaya_ai_v6_failure_analysis.json"
CASES_FILE = ROOT / "tests" / "fixtures" / "expanded_cases.json"


def _load_benchmark_module():
    module_path = ROOT / "scripts" / "benchmark_v6_full.py"
    spec = importlib.util.spec_from_file_location("benchmark_v6_full", module_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules["benchmark_v6_full"] = module
...
def main() -> int:
    ...
    if not failures_path.exists():
        raise SystemExit(f"Missing failure analysis: {failures_path}")
    ...
    if override_cases:
        ...
        failed_ids = [str(item["id"]) for item in failure_data.get("failures", []) if "id" in item]
    ...
    for cid in failed_ids:
        ...
        response = benchmark.generate_v6_full_response(case["input"])
